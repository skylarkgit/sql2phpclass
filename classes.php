...
class address implements dbconn{
	...
	function getLocalById($paddrid){
		$this->addrid=$paddrid;
		$stmt=$this->db->prepare('SELECT street,city,state,PIN,addrid FROM address WHERE addrid=:addrid');
		$stmt->bindParam(':addrid',$this->addrid);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'address : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class bill implements dbconn{
	...
	function getLocalById($pbillId){
		$this->billId=$pbillId;
		$stmt=$this->db->prepare('SELECT chartId,exchangeId,billId FROM bill WHERE billId=:billId');
		$stmt->bindParam(':billId',$this->billId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'bill : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class categories implements dbconn{
	...
	function getLocalById($pcategoryId){
		$this->categoryId=$pcategoryId;
		$stmt=$this->db->prepare('SELECT name,categoryId FROM categories WHERE categoryId=:categoryId');
		$stmt->bindParam(':categoryId',$this->categoryId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'categories : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class charts implements dbconn{
	...
	function getLocalById($pchartId){
		$this->chartId=$pchartId;
		$stmt=$this->db->prepare('SELECT availId,sessionId,status,exchangeId,chartId FROM charts WHERE chartId=:chartId');
		$stmt->bindParam(':chartId',$this->chartId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'charts : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class contact implements dbconn{
	...
	function getLocalById($pcontactId){
		$this->contactId=$pcontactId;
		$stmt=$this->db->prepare('SELECT email,phone1,phone2,contactId FROM contact WHERE contactId=:contactId');
		$stmt->bindParam(':contactId',$this->contactId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'contact : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class doctors implements dbconn{
	...
	function getLocalById($pdoctorId){
		$this->doctorId=$pdoctorId;
		$stmt=$this->db->prepare('SELECT userId,profileId,doctorId FROM doctors WHERE doctorId=:doctorId');
		$stmt->bindParam(':doctorId',$this->doctorId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'doctors : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class employees implements dbconn{
	...
	function getLocalById($pempId){
		$this->empId=$pempId;
		$stmt=$this->db->prepare('SELECT userId,profileId,empId FROM employees WHERE empId=:empId');
		$stmt->bindParam(':empId',$this->empId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'employees : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class exchange implements dbconn{
	...
	function getLocalById($pexchangeId){
		$this->exchangeId=$pexchangeId;
		$stmt=$this->db->prepare('SELECT type,amount,discount,discountType,exchangeId,net FROM exchange WHERE exchangeId=:exchangeId');
		$stmt->bindParam(':exchangeId',$this->exchangeId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'exchange : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class expenditures implements dbconn{
	...
	function getLocalById($pexpenditureId){
		$this->expenditureId=$pexpenditureId;
		$stmt=$this->db->prepare('SELECT expenseId,exchangeId,expenditureId FROM expenditures WHERE expenditureId=:expenditureId');
		$stmt->bindParam(':expenditureId',$this->expenditureId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'expenditures : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class expenses implements dbconn{
	...
	function getLocalById($pexpenseId){
		$this->expenseId=$pexpenseId;
		$stmt=$this->db->prepare('SELECT name,cost,expenseId FROM expenses WHERE expenseId=:expenseId');
		$stmt->bindParam(':expenseId',$this->expenseId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'expenses : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class groups implements dbconn{
	...
	function getLocalById($pgroupId){
		$this->groupId=$pgroupId;
		$stmt=$this->db->prepare('SELECT groupName,groupId FROM groups WHERE groupId=:groupId');
		$stmt->bindParam(':groupId',$this->groupId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'groups : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class lab implements dbconn{
	...
	function getLocalById($plabId){
		$this->labId=$plabId;
		$stmt=$this->db->prepare('SELECT serviceId,paramId,cost,labId FROM lab WHERE labId=:labId');
		$stmt->bindParam(':labId',$this->labId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'lab : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class params implements dbconn{
	...
	function getLocalById($pparamId){
		$this->paramId=$pparamId;
		$stmt=$this->db->prepare('SELECT paramId,paramString FROM params WHERE paramId=:paramId');
		$stmt->bindParam(':paramId',$this->paramId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'params : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class patientgroups implements dbconn{
	...
	function getLocalById($p){
		
		$stmt=$this->db->prepare('SELECT groupId,patientId FROM patientgroups WHERE ');
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'patientgroups : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class patients implements dbconn{
	...
	function getLocalById($ppatientId){
		$this->patientId=$ppatientId;
		$stmt=$this->db->prepare('SELECT userId,patientId FROM patients WHERE patientId=:patientId');
		$stmt->bindParam(':patientId',$this->patientId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'patients : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class personalinfo implements dbconn{
	...
	function getLocalById($pinfoId){
		$this->infoId=$pinfoId;
		$stmt=$this->db->prepare('SELECT name,gender,bloodGroup,dob,aadhar,infoId FROM personalinfo WHERE infoId=:infoId');
		$stmt->bindParam(':infoId',$this->infoId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'personalinfo : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class profiles implements dbconn{
	...
	function getLocalById($pprofileId){
		$this->profileId=$pprofileId;
		$stmt=$this->db->prepare('SELECT type,name,description,profileId FROM profiles WHERE profileId=:profileId');
		$stmt->bindParam(':profileId',$this->profileId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'profiles : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class service implements dbconn{
	...
	function getLocalById($pserviceId){
		$this->serviceId=$pserviceId;
		$stmt=$this->db->prepare('SELECT name,cost,description,categoryId,serviceId FROM service WHERE serviceId=:serviceId');
		$stmt->bindParam(':serviceId',$this->serviceId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'service : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class serviceavailed implements dbconn{
	...
	function getLocalById($pavailId){
		$this->availId=$pavailId;
		$stmt=$this->db->prepare('SELECT patientId,serviceId,exchangeId,taxId,availId FROM serviceavailed WHERE availId=:availId');
		$stmt->bindParam(':availId',$this->availId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'serviceavailed : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class servicecategories implements dbconn{
	...
	function getLocalById($pcategoryId){
		$this->categoryId=$pcategoryId;
		$stmt=$this->db->prepare('SELECT name,categoryId FROM servicecategories WHERE categoryId=:categoryId');
		$stmt->bindParam(':categoryId',$this->categoryId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'servicecategories : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class session implements dbconn{
	...
	function getLocalById($psessionId){
		$this->sessionId=$psessionId;
		$stmt=$this->db->prepare('SELECT timeStamp,dateAndTime,duration,sessionId FROM session WHERE sessionId=:sessionId');
		$stmt->bindParam(':sessionId',$this->sessionId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'session : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class taxes implements dbconn{
	...
	function getLocalById($ptaxId){
		$this->taxId=$ptaxId;
		$stmt=$this->db->prepare('SELECT taxName,percent,taxId FROM taxes WHERE taxId=:taxId');
		$stmt->bindParam(':taxId',$this->taxId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'taxes : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class transactions implements dbconn{
	...
	function getLocalById($ptxnId){
		$this->txnId=$ptxnId;
		$stmt=$this->db->prepare('SELECT type,status,exchangeId,txnId FROM transactions WHERE txnId=:txnId');
		$stmt->bindParam(':txnId',$this->txnId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'transactions : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
...
class users implements dbconn{
	...
	function getLocalById($puserId){
		$this->userId=$puserId;
		$stmt=$this->db->prepare('SELECT infoId,contactId,addrId,userId FROM users WHERE userId=:userId');
		$stmt->bindParam(':userId',$this->userId);
		if($stmt->execute()==false) {
			$this->errorLog($stmt->errorInfo()[2]);
			return new RESPONSE(null,'users : SELECT LOCAL BY ID','ERROR');
		}
		$retObj=$stmt->fetch(PDO::FETCH_OBJ);
		return new RESPONSE($retObj,'SUCCESS','OK');
	}
